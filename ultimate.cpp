...
// ── DXY / XAU consolidation snapshots — store daily-% change at each confirmed consolidation ──
static double   dxySnapshot        = 0;   // DXY daily % change captured at last consolidation
static double   xauSnapshot        = 0;   // XAU daily % change captured at last consolidation
static double   lastConsolidationDXYDelta = 0;  // DXY daily% delta at the most recent consolidation mark
static double   lastConsolidationXAUDelta = 0;  // XAU daily% delta at the most recent consolidation mark
static double   lastConsolidationXAUPct   = 0;  // XAU daily% at the most recent consolidation mark
static double   prevConsolidationDXYDelta = 0;  // DXY delta from the consolidation before the most recent
static double   prevConsolidationXAUDelta = 0;  // XAU delta from the consolidation before the most recent
static datetime lastTradeDayDate   = 0; // tracks the day of the last trade taken
static int      rPendingDir        = 0; // R1/R2 armed: -1=SELL, 1=BUY, 0=none
static int      rWatchBars         = 0; // bars elapsed since R1/R2 was armed
static int      r34PendingDir      = 0; // R3/R4 armed: -1=SELL(R3), 1=BUY(R4), 0=none
static int      r34WatchBars       = 0; // bars elapsed since R3/R4 was armed
static double   r34TriggerPrice    = 0; // ceiling (R3) or floor (R4) to watch for
static int      r56PendingDir      = 0; // R5/R6 armed: -1=SELL(R5), 1=BUY(R6), 0=none
static int      r56WatchBars       = 0; // bars elapsed since R5/R6 was armed
static double   r56TriggerPrice    = 0; // ceiling (R5) or floor (R6) to watch for
...
input int    MinConsolidationBars       = 5;    // minimum candles that must touch the floor/ceiling (reference scan)
input int    ConsolidationWindowBars    = 10;   // sliding window size for reference scan
...
input bool   EnableR1R2                 = true;   // enable Rule 1 and Rule 2
input bool   EnableR3R4                 = true;   // enable Rule 3 and Rule 4
input bool   EnableR5R6                 = true;   // enable Rule 5 and Rule 6
...
void DrawConsolidationMark(datetime t, double price, bool isStart, bool isSell)
{
   ...
   if (isStart)
   {
      // One label per unique anchor time — global cooldown prevents overlapping labels
      // regardless of direction (sell+buy firing at same spot would still stack)
      static datetime lastSellMarkT = 0;
      static datetime lastBuyMarkT  = 0;
      static datetime lastAnyMarkT  = 0;
      if (isSell) {
         if (t == lastSellMarkT) return;
         lastSellMarkT = t;
      } else {
         if (t == lastBuyMarkT) return;
         lastBuyMarkT = t;
      }
      if (t == lastAnyMarkT) return;   // other direction just drew at same anchor — skip
      lastAnyMarkT = t;

      // Daily % change for DXY: (curDXY - dailyOpenDXY) / dailyOpenDXY * 100
      ...
      double dxyDelta    = curDXYPct - dxySnapshot;   // change in daily% since last consolidation
      prevConsolidationDXYDelta = lastConsolidationDXYDelta;
      lastConsolidationDXYDelta = dxyDelta;

      // Daily % change for XAU: (curXAU - dailyOpenXAU) / dailyOpenXAU * 100
      ...
      lastConsolidationXAUPct = curXAUPct;
      double xauDelta    = curXAUPct - xauSnapshot;
      prevConsolidationXAUDelta = lastConsolidationXAUDelta;
...
void CleanupAllMarks()
{
   ObjectsDeleteAll(0, "DXYMark_");
   ObjectsDeleteAll(0, "SC_");
   ObjectsDeleteAll(0, "BC_");
   ObjectsDeleteAll(0, "PZ_");
}
...
// DXY computed from daily-open prices of each component — used as the fixed daily reference
double CalcDXYDailyOpen()
{
   double eurusd = iOpen("EURUSD", PERIOD_D1, 0);
   double usdjpy = iOpen("USDJPY", PERIOD_D1, 0);
   double gbpusd = iOpen("GBPUSD", PERIOD_D1, 0);
   double usdcad = iOpen("USDCAD", PERIOD_D1, 0);
   double usdsek = iOpen("USDSEK", PERIOD_D1, 0);
   double usdchf = iOpen("USDCHF", PERIOD_D1, 0);
...
{
   // No-op: dxySnapshot is now captured in DrawConsolidationMark on each confirmed consolidation.
}

void UpdateChangeSnapshot()
{
   // No-op: snapshot is now captured at each trade entry in ExecMarket.
}

void PrintChangeStats()
{
   double curDXY     = CalcDXY();
   double dxyOpenVal = CalcDXYDailyOpen();
   double curDXYPct  = (dxyOpenVal > 0 && curDXY > 0) ? ((curDXY - dxyOpenVal) / dxyOpenVal * 100.0) : 0;
   double dxyDelta   = curDXYPct - dxySnapshot;
...
void ExecMarket(int dir)
{
   ...
   inTrade          = true;
   peakProfit       = 0.0;
   trendingPeakProfit = 0.0;
   trailingActive   = false;
   breakEvenDone    = false;
   entryCount++;
   exitCooldownBars = MinConsolidationBars;
   lastTradeDayDate = StringToTime(TimeToString(TimeCurrent(), TIME_DATE));
   DrawEntry(dir == 1);
...
void OnTick()
{
   ...
   if (posCount > 0)
   {
      ...
      {
         Print("CLOSE_TP: profit=", DoubleToString(accountProfit,2), " >= TP=", TakeProfitDollars);
         consecutiveLosses = 0;
         CloseAll();
         ResetAll();
         return;
      ...
      Print("TICK: profit=", DoubleToString(accountProfit,2),
            " peak=", DoubleToString(peakProfit,2),
            " trailingActive=", trailingActive,
            " breakEvenDone=", breakEvenDone,
            " exitCooldown=", exitCooldownBars,
            " exitPendingDir=", exitPendingDir);
      ...
      if (peakProfit >= StopLossDollars * TrailingActivateFactor)
      {
         ...
         trailingActive = true;
         // Lock in a fixed fraction of peak profit — scales with the move, never fixed-tight
         double lockInAmount = peakProfit * TrailingRetracementFactor;
         ...
         for (int i = 0; i < PositionsTotal(); i++)
         {
            ...
            {
               // Only move SL if current profit still exceeds what we're locking in —
               // prevents SL from being placed above current price during a retracement
               if (accountProfit <= lockInAmount)
               {
                  Print("TRAIL_SKIP: profit=", DoubleToString(accountProfit,2), " <= lockIn=", DoubleToString(lockInAmount,2));
                  continue;
               }
               double trailingSL = DollarsToPriceDist(lockInAmount);
               newSL = (posType == POSITION_TYPE_BUY)
                       ? openPrice + trailingSL
                       : openPrice - trailingSL;
               Print("TRAIL_MOVE: newSL=", DoubleToString(newSL,_Digits), " lockIn=$", DoubleToString(lockInAmount,2), " peak=$", DoubleToString(peakProfit,2));
            }
            MoveSLToPrice(newSL);
         }
      }
   ...
   // ── R1/R2 EMA20–EMA50 zone watcher (tick-level) ──────────────────────────
   // Fires when price enters the zone between EMA20 and EMA50
   if (!inTrade && rPendingDir != 0 && EnableR1R2)
   {
      int h20 = iMA(_Symbol, _Period, 20, 0, MODE_EMA, PRICE_CLOSE);
      ...
      if (h20 != INVALID_HANDLE && h50 != INVALID_HANDLE)
      {
         ...
         if (CopyBuffer(h20, 0, 0, 1, ema20buf) > 0 && CopyBuffer(h50, 0, 0, 1, ema50buf) > 0)
         {
            double ema20 = ema20buf[0];
            double ema50 = ema50buf[0];
            double ask   = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            double bid   = SymbolInfoDouble(_Symbol, SYMBOL_BID);

            double lo = MathMin(ema20, ema50);
            double hi = MathMax(ema20, ema50);
            bool inZone = (rPendingDir == -1) ? (bid >= lo && bid <= hi)
                                              : (ask >= lo && ask <= hi);

            if (inZone)
            {
               int dir = rPendingDir;
               rPendingDir = 0; rWatchBars = 0;
               lastEntryRule = (dir == -1) ? "R1_SELL_EMA_ZONE" : "R2_BUY_EMA_ZONE";
               Print((dir == -1 ? "R1_ENTRY SELL" : "R2_ENTRY BUY"),
   ...
   // ── R3/R4 ceiling/floor touch watcher (tick-level) ───────────────────────
   // R3 SELL: fires when price rises to/above consolidation ceiling
   // R4 BUY:  fires when price falls to/below consolidation floor
   if (!inTrade && r34PendingDir != 0 && r34TriggerPrice > 0 && EnableR3R4)
   {
      double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      bool hit = (r34PendingDir == -1 && bid >= r34TriggerPrice) ||
                 (r34PendingDir ==  1 && ask <= r34TriggerPrice);
      if (hit)
      {
         int    dir     = r34PendingDir;
         double trigger = r34TriggerPrice;
         r34PendingDir = 0; r34WatchBars = 0; r34TriggerPrice = 0;
         lastEntryRule = (dir == -1) ? "R3_SELL" : "R4_BUY";
         Print((dir == -1 ? "R3_ENTRY SELL" : "R4_ENTRY BUY"),
               ": price=", DoubleToString(dir == -1 ? bid : ask, _Digits),
   ...
   {
      rWatchBars++;
      if (rWatchBars > EntryMaxWaitBars)
      {
         Print("R", (rPendingDir == -1 ? "1" : "2"), "_EXPIRED: bars=", rWatchBars);
         rPendingDir = 0; rWatchBars = 0;
      }
   }
   if (r34PendingDir != 0)
   {
      r34WatchBars++;
      if (r34WatchBars > EntryMaxWaitBars)
      {
         Print("R", (r34PendingDir == -1 ? "3" : "4"), "_EXPIRED: bars=", r34WatchBars);
         r34PendingDir = 0; r34WatchBars = 0; r34TriggerPrice = 0;
      }
   }
   if (r56PendingDir != 0)
   {
      r56WatchBars++;
      if (r56WatchBars > EntryMaxWaitBars)
      {
         Print("R", (r56PendingDir == -1 ? "5" : "6"), "_EXPIRED: bars=", r56WatchBars);
   ...
   if (inTrade && posCount == 0)
   {
      ...
      Print("TRADE_CLOSED: rule=", lastEntryRule,
   ...
   // ── Idle — detect consolidation and arm R1/R2 ───────────────────────────
   if (!inTrade && pendingDir == 0)
   {
      ...
      if (zoneDir == 0) return;  // no new consolidation this bar

      // Get EMA200 for position check
      double ema200 = 0;
      {
         int h200 = iMA(_Symbol, _Period, 200, 0, MODE_EMA, PRICE_CLOSE);
         if (h200 != INVALID_HANDLE) {
            double buf[1]; ArraySetAsSeries(buf, true);
            if (CopyBuffer(h200, 0, 0, 1, buf) > 0) ema200 = buf[0];
            IndicatorRelease(h200);
         }
      }
      double curPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);

      // ── BUY consolidation, price BELOW EMA200 → R1 (SELL via EMA zone) + R6 (BUY at floor) ──
      if (zoneDir == 1 && ema200 > 0 && curPrice < ema200)
      {
         // R1: DXY ok + XAU negative → arm SELL, wait for EMA20–EMA50 zone
         if (EnableR1R2)
         {
            bool dxyOk  = (lastConsolidationDXYDelta > 0) ||
                          (lastConsolidationDXYDelta < 0 && lastConsolidationDXYDelta > lastConsolidationXAUDelta);
            bool xauNeg = (lastConsolidationXAUDelta < 0);
            if (dxyOk && xauNeg)
            {
               rPendingDir = -1; rWatchBars = 0;
               Print("R1_SELL_ARMED: price below EMA200=", DoubleToString(ema200, _Digits),
                     " DXY delta=", DoubleToString(lastConsolidationDXYDelta, 3),
                     "% XAU delta=", DoubleToString(lastConsolidationXAUDelta, 3),
                     "% waiting for price in EMA20-EMA50 zone");
            }
            else
               Print("R1_SELL_SKIP: dxyOk=", dxyOk, " xauNeg=", xauNeg,
      ...
      // ── SELL consolidation, price ABOVE EMA200 → R2 (BUY via EMA zone) + R5 (SELL at ceiling) ──
      else if (zoneDir == -1 && ema200 > 0 && curPrice > ema200)
      {
         // R2: XAU ok + DXY negative → arm BUY, wait for EMA20–EMA50 zone
         if (EnableR1R2)
         {
            bool xauOk  = (lastConsolidationXAUDelta > 0) ||
                          (lastConsolidationXAUDelta < 0 && lastConsolidationXAUDelta > lastConsolidationDXYDelta);
            bool dxyNeg = (lastConsolidationDXYDelta < 0);
            if (xauOk && dxyNeg)
            {
               rPendingDir = 1; rWatchBars = 0;
               Print("R2_BUY_ARMED: price above EMA200=", DoubleToString(ema200, _Digits),
                     " XAU delta=", DoubleToString(lastConsolidationXAUDelta, 3),
                     "% DXY delta=", DoubleToString(lastConsolidationDXYDelta, 3),
                     "% waiting for price in EMA20-EMA50 zone");
            }
            else
               Print("R2_BUY_SKIP: xauOk=", xauOk, " dxyNeg=", dxyNeg,
                     " XAU=", DoubleToString(lastConsolidationXAUDelta, 3),
                     "% DXY=", DoubleToString(lastConsolidationDXYDelta, 3), "%");
         }
      ...
      // ── SELL consolidation, price BELOW EMA200 → R3 (SELL at ceiling) ───────
      else if (zoneDir == -1 && ema200 > 0 && curPrice < ema200)
      {
         if (EnableR3R4)
         {
            ...
            if (dxyOk && xauNeg)
            {
               r34PendingDir = -1; r34WatchBars = 0; r34TriggerPrice = newHi;
               Print("R3_SELL_ARMED: price below EMA200=", DoubleToString(ema200, _Digits),
                     " ceiling=", DoubleToString(newHi, _Digits),
                     " DXY delta=", DoubleToString(lastConsolidationDXYDelta, 3),
                     "% XAU delta=", DoubleToString(lastConsolidationXAUDelta, 3),
                     "% waiting for price to touch/exceed ceiling");
            }
            else
               Print("R3_SELL_SKIP: dxyOk=", dxyOk, " xauNeg=", xauNeg,
                     " DXY=", DoubleToString(lastConsolidationDXYDelta, 3),
                     "% XAU=", DoubleToString(lastConsolidationXAUDelta, 3), "%");
         }
      }

      // ── BUY consolidation, price ABOVE EMA200 → R4 (BUY at floor) ───────────
      else if (zoneDir == 1 && ema200 > 0 && curPrice > ema200)
      {
         if (EnableR3R4)
         {
            ...
            if (xauOk && dxyNeg)
            {
               r34PendingDir = 1; r34WatchBars = 0; r34TriggerPrice = newLo;
               Print("R4_BUY_ARMED: price above EMA200=", DoubleToString(ema200, _Digits),
                     " floor=", DoubleToString(newLo, _Digits),
                     " XAU delta=", DoubleToString(lastConsolidationXAUDelta, 3),
                     "% DXY delta=", DoubleToString(lastConsolidationDXYDelta, 3),
                     "% waiting for price to touch/drop below floor");
            }
            else
               Print("R4_BUY_SKIP: xauOk=", xauOk, " dxyNeg=", dxyNeg,
                     " XAU=", DoubleToString(lastConsolidationXAUDelta, 3),
                     "% DXY=", DoubleToString(lastConsolidationDXYDelta, 3), "%");
         }
      }
